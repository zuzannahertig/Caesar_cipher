...
class Cipher(ABC):
    CHARACTERS: List[str] = [string.ascii_lowercase, string.ascii_uppercase, string.punctuation]
    ...
    def shift(self, value: int) -> None:
        if not isinstance(value, int):
            ...
            warnings.warn('Shift of value 0 will result in returning text in an unchanged form.', TextNotEncrypted)
        elif value < 0:
...
class Encryptor(Cipher):
    ...
    def cypher(self, text: str) -> str:
        """Encrypt text."""
        final_shifted_alphabet = super().cypher(text)
